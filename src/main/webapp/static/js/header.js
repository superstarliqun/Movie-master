...
//初始化
function initHeader(){
    var LayuiNavMore = $(".header-li");//头像摆放位置
    var user_json = JSON.parse(localStorage.getItem("userJson"));

    layui.use('/Movie/static/layui/lay/modules/element', function(){
    ...
    if(user_json == null){
        LayuiNavMore.append(
            "<a href=\"javascript:;\" style=\"padding: 0;height: 42px; width: 42px;\"><img src=\"/Movie/static/images/head.jpg\" class=\"layui-nav-img\"></a>" +
            "<dl class=\"layui-nav-child nav-image\">" +
            "<dd><a href=\"/Movie/views/view/login.html\">登录</a></dd>" +
            "</dl>"
        );
    }else{
        var HeadImg = "";
        if(user_json.user_headImg == null || typeof user_json.user_headImg == "undefined"){
            HeadImg = "/Movie/upload/head/demo.jpg";
        }else{
        ...
        ;LayuiNavMore.append(
            ...
            "<dl class=\"layui-nav-child nav-image\">" +
            "<dd><a href=\"/Movie/views/view/center.html\" onclick=\"mycenter()\">我的订单</a></dd>" +
            "<hr/>" +
            "<dd><a href=\"/Movie/views/view/center.html\" onclick=\"myinformation()\">基本信息</a></dd>" +
            "<hr/>" +
